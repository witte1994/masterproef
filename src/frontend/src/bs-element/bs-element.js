 ...
 * @polymer
 */
class BsElement extends PolymerElement {
    static get template() {
        return html`
        ...
        <iron-ajax
            id="ajaxBs"
            url="http://localhost:3000/user/[[userId]]/bs/[[startInt]]\&[[endInt]]"
            method="GET"
            handle-as="json"
    ...
    ready() {
        ...
        this.$.ajaxBsSmall.generateRequest();
    }
    ...
    dataReceived(event) {
        ...
        var valArray = ['Blood sugar (mmol/L)'];
        var dateArray = ['x'];
        ...
        this.generateChart(dateArray, valArray, thresholds);
    }

    generateChart(dateArray, valArray, thresholds) {
    ...
    resize(e) {
        this.dispatchEvent(new CustomEvent('resize', { composed: true, detail: { resizeTo: "bs-element-small" }}));
    }
...
window.customElements.define('bs-element', BsElement);
